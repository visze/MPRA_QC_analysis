...
def create_allelic_pairs_df(reporter_experiment_barcode_file, comparative_map_file, output_file):
    ...
    allele1_map = activity_df.rename(
        columns={"seq_id": "REF", **{col: f"{col}_allele1" for col in cols}}
    )
    allele2_map = activity_df.rename(
        columns={"seq_id": "ALT", **{col: f"{col}_allele2" for col in cols}}
    )
