...
def plot_retained_cCREs_and_barcodes(result_melted_df):
    ...
    legend2 = ax.legend(handles=rep_handles, title="Replicate", loc="upper left", bbox_to_anchor=(1.02, 0.77), frameon=True)
...
def plot_activity_distribution(act_df):
    ...
    plt.text(
        0.05,
        0.95,  # relative position in axes coordinates
        f"Skew test:\nstat = {stat:.2f}\n{p_text}",
        ha="left",
        va="top",
...
def plot_reproducibility_by_sequencing_depth(ds_activity_path, ds_ratio_path):
    ...
    sns.lineplot(
        data=summary_df, x="Sampling parameter", y="non_active", color="gray", marker="o", label="Not active", alpha=0.6
    )
...
def plot_activity_statistic_vs_count_ratio(act_df):
    ...
    plt.clf()
    scatter = plt.scatter(x, y, c=density_capped, cmap=custom_cmap_bolder, s=10, edgecolors="none")  # use capped values
    ax = plt.gca()
    plt.xlabel(r"$\log_{2}\!\left(\frac{\mathrm{RNA}}{\mathrm{DNA}}\right)$")
