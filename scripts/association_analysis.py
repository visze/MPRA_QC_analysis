...
def BCs_per_cCRE_plot(final_counts_df):
    ...
    plt.text(
        plt.gca().get_xlim()[1] * 0.4,
        frac_at_zero + 0.02,  # position text near right side
        f"cCREs with 0 barcodes: {frac_at_zero:.2%}",
        color=plot_color_pallete["cCRE"],
    )
    plt.text(
        plt.gca().get_xlim()[1] * 0.4,
        frac_at_ten + 0.05,  # position text near right side
        f"cCREs with fewer than 10 barcodes: {frac_at_ten:.2%}",
        color=plot_color_pallete["cCRE"],
    )
    plt.text(
        med * 2,
        0.55,  # position text near right side
        f"Median number of barcodes: {med:d}",
        color=plot_color_pallete["barcode"],
    )
    plt.text(
        avg * 2,
        0.65,  # position text near right side
...
def PCR_bias_G_stretches_plot(final_counts_df):
    ...
    data = [final_counts_df.loc[final_counts_df["g_stretch"] == x, "association_count"].to_numpy() for x in xs]
